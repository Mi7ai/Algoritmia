...
def binpacking_solve(objects: List[int], capacity: int):
    class BinPackingBabPS(BabPartialSolution):
        ...
        def calc_opt_bound(self) -> Union[int, float]:
            ...
            free = sum(capacity - w for w in self.container_weights if capacity - w >= min_object)  # hueco libre
            ...
            return len(self.container_weights) + ceil((sum(pending) - in_free)/capacity)
